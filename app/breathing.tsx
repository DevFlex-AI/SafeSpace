// Breathing Exercise — 4-4-6 pattern with animated circle + sound
...
export default function BreathingScreen() {
  ...
  // Initialize sounds
  useEffect(() => {
    initBreathingSounds().then(() => setSoundsReady(true));
    return () => { unloadBreathingSounds(); };
  }, []);
  ...
  useEffect(() => {
    if (phase === 'inhale') {
      ...
      if (soundEnabled && soundsReady) playInhale();
    } else if (phase === 'hold') {
      ...
      if (soundEnabled && soundsReady) playHold();
    } else if (phase === 'exhale') {
      ...
      if (soundEnabled && soundsReady) playExhale();
    } else if (phase === 'done') {
      ...
      <View style={styles.content}>
            ...
            <View style={styles.circleInner}>
              {phase !== 'idle' && phase !== 'done' ? (
                ...
                <>
                  ...
                  <Text style={[styles.phaseText, { color: theme.accent, marginTop: 8 }]}>{getPhaseLabel()}</Text>
                </>
              ) : (
          ...
          {isActive ? (
            <View style={styles.cycleDots}>
              {Array.from({ length: defaultCycles }).map((_, i) => (
                <View key={i} style={[styles.cycleDot, i <= currentCycle && { backgroundColor: theme.primary }]} />
              ))}
            </View>
          ) : null}
        </View>
        ...
        <View style={styles.controls}>
          {phase === 'idle' ? (
            ...
            <Pressable style={styles.stopBtn} onPress={resetExercise}>
        ...
        <View style={styles.soundIndicator}>
          <MaterialIcons name={soundEnabled ? 'volume-up' : 'volume-off'} size={14} color={theme.textMuted} />
          <Text style={styles.soundIndicatorText}>Sound {soundEnabled ? 'on' : 'off'}</Text>
        </View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  circleWrap: { alignItems: 'center', marginBottom: 48 },
  circle: { width: CIRCLE_SIZE, height: CIRCLE_SIZE, borderRadius: CIRCLE_SIZE / 2, backgroundColor: theme.primaryLight, alignItems: 'center', justifyContent: 'center' },
  circleInner: { alignItems: 'center', justifyContent: 'center' },
  timerText: { fontSize: 72, fontWeight: '300', letterSpacing: -2 },
  phaseText: { fontSize: 18, fontWeight: '600', color: theme.textSecondary },
  cycleDots: { flexDirection: 'row', gap: 8, marginTop: 24 },
  cycleDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: theme.border },
  controls: { alignItems: 'center', marginBottom: 24 },
  startBtn: { backgroundColor: theme.primary, paddingHorizontal: 48, paddingVertical: 16, borderRadius: theme.radius.full },
  startBtnText: { fontSize: 18, fontWeight: '600', color: '#FFF' },
  stopBtn: { paddingHorizontal: 32, paddingVertical: 14, borderRadius: theme.radius.full, borderWidth: 2, borderColor: theme.border },
  stopBtnText: { fontSize: 16, fontWeight: '600', color: theme.textSecondary },
  doneControls: { flexDirection: 'row', gap: 16 },
  infoText: { fontSize: 14, color: theme.textMuted, textAlign: 'center', lineHeight: 20 },
  soundIndicator: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 16, opacity: 0.6 },
  soundIndicatorText: { fontSize: 12, color: theme.textMuted },
});
