// Mood Screen — Mood tracking, history, weekly insights + journal
...
export default function MoodScreen() {
  ...
  const distribution = MOOD_OPTIONS.map(opt => ({
    ...
    percentage: last30.length > 0
      ? Math.round((last30.filter(m => m.value === opt.value).length / last30.length) * 100) : 0,
  }));
  ...
  const saveJournal = () => {
    if (selectedMood === null) return;
    const fullNote = journalText.trim() ? `[Journal] ${journalText.trim()}` : note;
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    addMoodEntry(selectedMood, fullNote);
    setSelectedMood(null);
    setNote('');
    setJournalText('');
    setShowInput(false);
  ...
  for (let i = 0; i < uniqueDates.length; i++) {
    ...
    if (uniqueDates.includes(expectedStr)) { moodStreak++; } else break;
  }
        ...
        <View style={styles.journalOverlay}>
          <View style={[styles.journalCard, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            <View style={styles.journalHeader}>
              ...
              <Pressable onPress={() => setShowJournal(false)} style={styles.journalClose}>
            ...
            <Text style={styles.journalCount}>{journalText.length}/1000</Text>

            <View style={styles.journalActions}>
              <Pressable style={styles.journalSecondaryBtn} onPress={() => setShowJournal(false)}>
                <Text style={styles.journalSecondaryBtnText}>Cancel</Text>
              </Pressable>
              <Pressable style={styles.journalSaveBtn} onPress={saveJournal}>
            ...
            <View style={styles.moodGrid}>
              {MOOD_OPTIONS.map((option) => {
                ...
                return (
                  <Pressable
                    key={option.value}
                    style={[styles.moodBtn, isSelected && { backgroundColor: option.color + '25', borderColor: option.color, borderWidth: 2 }]}
                    onPress={() => handleSelectMood(option.value)}
                  >
                    <Text style={styles.moodBtnEmoji}>{option.emoji}</Text>
                    <Text style={[styles.moodBtnLabel, isSelected && { color: option.color, fontWeight: '700' }]}>{option.label}</Text>
                  </Pressable>
                );
              })}
            </View>
            {showInput ? (
              <Animated.View entering={FadeIn.duration(300)} style={styles.noteSection}>
                ...
                <View style={styles.moodActionRow}>
                  <Pressable style={styles.journalBtn} onPress={openJournal}>
                    <MaterialIcons name="edit-note" size={20} color={theme.primary} />
                    <Text style={styles.journalBtnText}>Write Journal</Text>
                  </Pressable>
              ...
              {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => {
                ...
                return (
                  <View key={`${day}-${i}`} style={styles.weekBarCol}>
                    {entry ? <Text style={styles.weekBarEmoji}>{MOOD_OPTIONS.find(m => m.value === entry.value)?.emoji}</Text> : null}
            ...
            {weeklyAverage > 0 ? (
              <View style={styles.avgRow}>
                ...
                <View style={styles.avgValue}>
                  <Text style={{ fontSize: 18 }}>{MOOD_OPTIONS.find(m => m.value === Math.round(weeklyAverage))?.emoji}</Text>
                  <Text style={styles.avgText}>{MOOD_OPTIONS.find(m => m.value === Math.round(weeklyAverage))?.label}</Text>
                </View>
              </View>
            ) : null}
          </View>
          ...
          <View style={styles.distCard}>
              ...
              <View key={item.value} style={styles.distRow}>
                ...
                <View style={styles.distBarWrap}>
                  <View style={styles.distBarBg}>
                    <View style={[styles.distBarFill, { width: `${Math.max(item.percentage, 2)}%`, backgroundColor: item.color }]} />
                  </View>
            ...
            {last30.length === 0 ? (
              <View style={styles.emptyDist}>
                <Image source={require('../../assets/images/mood-empty.png')} style={{ width: 160, height: 120 }} contentFit="contain" />
                    ...
                    <View style={styles.entryHeader}>
                      ...
                      {isJournal ? (
                        <View style={styles.journalTag}>
                          <MaterialIcons name="edit-note" size={12} color={theme.primary} />
                          <Text style={styles.journalTagText}>Journal</Text>
                        </View>
                      ) : null}
                    </View>
                    {displayNote ? <Text style={styles.entryNote} numberOfLines={3}>{displayNote}</Text> : null}
            ...
            <LinearGradient colors={['#EDE9FE', '#F3E8FF']} style={styles.insightCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.insightText}>
                  {weeklyAverage >= 4
                    ? "You have been feeling great this week! Keep up the positive habits that are working for you."
                    : weeklyAverage >= 3
                    ? "Your week has been mixed. That is completely normal. Try focusing on what made the good days feel good."
...
const styles = StyleSheet.create({
  ...
  moodBtn: { alignItems: 'center', padding: 10, borderRadius: theme.radius.lg, borderWidth: 2, borderColor: 'transparent', minWidth: 58, gap: 6 },
  moodBtnEmoji: { fontSize: 32 },
  moodBtnLabel: { fontSize: 12, fontWeight: '500', color: theme.textSecondary },
  noteSection: { marginTop: 16, gap: 12 },
  noteInput: { backgroundColor: theme.backgroundSecondary, borderRadius: theme.radius.md, padding: 14, fontSize: 15, color: theme.textPrimary, minHeight: 48, textAlignVertical: 'top' },
  moodActionRow: { flexDirection: 'row', gap: 10 },
  journalBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.primarySoft, borderRadius: theme.radius.md, paddingVertical: 12, gap: 6, borderWidth: 1, borderColor: theme.primary + '30' },
  ...
  weekBar: { width: 20, borderRadius: 10, minHeight: 6 },
  weekBarLabel: { fontSize: 12, fontWeight: '600', color: theme.textMuted, marginTop: 4 },
  avgRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: theme.borderLight, paddingTop: 14, marginTop: 14 },
  ...
  distLabel: { fontSize: 12, color: theme.textSecondary },
  ...
  entryLabel: { fontSize: 16, fontWeight: '600', color: theme.textPrimary },
  journalTag: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.primarySoft, paddingHorizontal: 8, paddingVertical: 2, borderRadius: theme.radius.sm, gap: 3 },
  journalTagText: { fontSize: 11, fontWeight: '600', color: theme.primary },
  entryNote: { fontSize: 13, color: theme.textSecondary, marginTop: 2, lineHeight: 18 },
